...
Hooks.once("init", () => {
    ...
    mask.on("updateStage", (mask) => {
        for (const token of canvas.tokens.placeables) {
            if (!token.visible || !token.renderable || !token.icon || !token.icon.parent || !token.icon.visible || !token.renderable || token.icon.alpha < 1) {
                continue;
            }
        ...
        for (const tile of canvas.foreground.tiles) {
            if (!tile.visible || !tile.renderable || !tile.tile || !tile.tile.parent || !tile.tile.visible || !tile.tile.renderable || Tiles.getAlpha(tile) < 1 && Tiles.getOcclusionAlpha(tile) < 1) {
                continue;
            }
