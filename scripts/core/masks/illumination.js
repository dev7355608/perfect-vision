...
Hooks.once("init", () => {
    ...
    Hooks.on("lightingRefresh", () => {
        ...
        if (canvas.lighting.globalLight) {
            mask.stage.background.beginFill(0x00FF00).drawShape(canvas.dimensions.sceneRect).endFill();
        }
    ...
    patch("PointSource.prototype.drawLight", "POST", function (c) {
        ...
        if (this.sourceType === "light") {
            if (this._pv_radius > 0) {
                if (!c._pv_fov) {
                    c._pv_fov = new SourcePolygonMesh(this._pv_fov, new IlluminationSourcePolygonMeshShader({
                        source: this,
                        tint: 0xFF0000
                    }));
                } else {
            ...
            if (this._pv_fovMono) {
                if (!c._pv_fovMono) {
                    c._pv_fovMono = new SourcePolygonMesh(this._pv_fovMono, new IlluminationSourcePolygonMeshShader({
                        source: this,
                        tint: 0x00FF00
                    }));
            ...
            if (this._pv_fovColor) {
                if (!c._pv_fovColor) {
                    c._pv_fovColor = new SourcePolygonMesh(this._pv_fovColor, new IlluminationSourcePolygonMeshShader({
                        source: this,
                        tint: 0xFF0000
                    }));
            ...
            if (this._pv_fovBrighten) {
                if (!c._pv_fovBrighten) {
                    c._pv_fovBrighten = new SourcePolygonMesh(this._pv_fovBrighten, new IlluminationSourcePolygonMeshShader({
                        source: this,
                        tint: 0x0000FF
                    }));
...
class IlluminationSourcePolygonMeshShader extends SourcePolygonMeshShader {
    ...
    static elevationFragment = `\
        ...
        void main()
        {
            ...
            if (uElevationRange.x <= elevation && elevation < uElevationRange.y) {
                gl_FragColor = uColor;
            } else {
    ...
    constructor(options = {}) {
        options = Object.assign({
            program: IlluminationSourcePolygonMeshShader.defaultProgram,
        }, options);

        const uniforms = IlluminationSourcePolygonMeshShader.defaultUniforms();
    ...
    update() {
        ...
        if (this.uniforms.uElevation) {
            Elevation.getSourceElevationRange(this.source, this.uniforms.uElevationRange);
        }
    }
}
